...
<template>
    <Link :href="route(`get-${type}`, type === 'album' ? { artist: subTitle, album: title } : type === 'artist' ? mbid ? { use: 'mbid', mbid } : { use: 'name', artist: title } : { mbid })">
        <div class="bg-gray-800 p-2 rounded-md m-2 w-[230px] h-[320px] max-sm:w-[150px] md:w-[150px] max-sm:h-[200px] md:h-[230px] max-sm:ml-1 max-sm:mr-1 rounded-md hover:bg-gray-600 cursor-pointer overflow-hidden">
            <div class="">
