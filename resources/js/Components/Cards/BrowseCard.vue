...
<template>
    <Link :href="type == 'system' ? route(tag) : route('get-tag', { tag: title })">
        <div v-if="randColor.color" :style="`background-color: ${randColor.color};`" class="bg-gray-800 p-6 mt-4 w-[190px] h-[190px] max-sm:w-[150px] max-sm:h-[200px] max-sm:ml-1 max-sm:mr-1 rounded-md m-2 hover:bg-gray-600 cursor-pointer relative overflow-hidden">
